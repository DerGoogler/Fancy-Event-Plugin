...
var fireworks = {
  ...
  init: function () {
    this._addEventListener(window, "resize", function () {
      return fireworks.resize.apply(fireworks);
    });
    this._addEventListener(window, "load", function () {
      return fireworks.start.apply(fireworks);
    });
  },
  ...
  _addExplosion: function () {
    var x = Math.floor(this._random(this._bodyWidth)),
      y = Math.floor((this._random(0.5) + 0.1) * this._bodyHeight),
  ...
  _move: function () {
    ...
    for (var i = 0, l = p.length; i < l; i++) {
      ...
      if (d.age > 1) d.x = d.y = 0;

      el.style.left = d.x + "px";
      el.style.top = d.y + "px";
      el.style.color = Math.random() * 0.5 + d.age >= 1 ? "transparent" : d.color;
